...
def newss(request):
    ...
    return render(request,'nwsss.html',{'ww':jhj,'x':x})
        ...
        if request.method == 'POST':
            ...
            for w in mgn:
                if w.Email == email and w.User_role == 'student':
                    ...
                    return render(request, 'reg_stud.html')
...
def edit_admin(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'adm_prof.html', {'gtt': gtt})
    return render(request, 'edit_admin.html',{'bb1':bb1,'um':um})
...
def add_chapter(request):
    ...
    if request.method == 'POST':
        ...
        if cou_tit == 'Nil':
            messages.success(request, 'Please select course')
...
def add_chapter_c1(request):
    gg = request.session['subj_nn']
    request.session['subj_nn1'] = 87
    request.session['court0'] = khk = request.POST.get('cou')
    if request.session['court0'] == 'Nil':
        messages.success(request,'Please select a course')
        return redirect('add_chapter_c0')
    gg = str(gg)
...
def st_sub_selnew2(request):
    if request.session['subj_nn1'] == 57:
        d = request.POST.get('subj')
    else:
        d = request.session['sub_n']
    if d == 'Nil':
        messages.success(request,'Please select subject')
        return redirect('stu_sub_selnew')
...
def disp_teach(request):
    ...
    if couu == 'Nil':
        request.session['subj_nn1'] = 44
        messages.success(request, 'Please select course')
        return redirect('st_sub_selnew2')
...
def feedback(request):
    ...
    if request.method == 'POST':
        ...
        teach1 = teach.split(";")
        if course=='nil' or subject=='nil' or teach=='nil'  or teach1=='nil':
            request.method = 'GET'
            messages.success(request,'pleas select all fileds')
